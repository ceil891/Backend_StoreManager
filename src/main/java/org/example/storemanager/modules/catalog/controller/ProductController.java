...
import org.example.storemanager.modules.catalog.dto.response.product.CreateProductResponse;
import org.example.storemanager.modules.catalog.dto.response.product.DeleteProductResponse;
import org.example.storemanager.modules.catalog.dto.response.product.ProductResponse;
...
public class ProductController {
    ...
    @PreAuthorize("@securityEvaluator.hasPermission('catalog:product:view')")
    public ResponseEntity<ApiResponse<List<org.example.storemanager.modules.inventory.controller.InventoryTrackingApiController.SerialNumberDTO>>> getProductSerials(
            @PathVariable Long productId) {
        Product product = productRepository.findById(productId)
                .filter(p -> !Boolean.TRUE.equals(p.getIsDeleted()))
                .orElseThrow(() -> new ResourceNotFoundException("Product", "id", productId));

        List<org.example.storemanager.modules.inventory.controller.InventoryTrackingApiController.SerialNumberDTO> serials = 
                serialNumberRepository.findByProductIdAndIsDeletedFalse(productId).stream()
                        .map(sn -> org.example.storemanager.modules.inventory.controller.InventoryTrackingApiController.SerialNumberDTO.builder()
                                .id(sn.getId())
