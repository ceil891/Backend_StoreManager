package org.example.storemanager.controller.logistics;

import lombok.RequiredArgsConstructor;
import org.example.storemanager.dto.response.common.ApiResponse;
import org.example.storemanager.entity.crm.Shipper;
import org.example.storemanager.entity.crm.DeliveryTrip;
import org.example.storemanager.entity.crm.Promotion;
import org.example.storemanager.entity.omnichannel.ShippingCarrier;
import org.example.storemanager.repository.crm.ShipperRepository;
import org.example.storemanager.repository.crm.DeliveryTripRepository;
import org.example.storemanager.repository.crm.PromotionRepository;
import org.example.storemanager.repository.omnichannel.ShippingCarrierRepository;
...
public class LogisticsController {
    ...
    public ResponseEntity<ApiResponse<DeliveryTrip>> createTrip(@RequestBody Map<String, Object> req) {
        ...
        if (trip.getOrder() == null) {
            List<org.example.storemanager.entity.sales.SaleOrder> orders = saleOrderRepository.findAll();
            if (!orders.isEmpty()) {
    ...
    public ResponseEntity<ApiResponse<ShippingCarrier>> updateCarrier(@PathVariable Long id, @RequestBody ShippingCarrier req) {
        ShippingCarrier carrier = shippingCarrierRepository.findById(id)
                .orElseThrow(() -> new org.example.storemanager.exception.ResourceNotFoundException("ShippingCarrier", "id", id));
    ...
    public ResponseEntity<ApiResponse<Shipper>> updateShipper(@PathVariable Long id, @RequestBody Shipper req) {
        Shipper shipper = shipperRepository.findById(id)
                .orElseThrow(() -> new org.example.storemanager.exception.ResourceNotFoundException("Shipper", "id", id));
