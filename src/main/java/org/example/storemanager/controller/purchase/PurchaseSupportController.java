...
import org.example.storemanager.dto.response.purchase.PurchaseDashboardSummaryResponse;
...
import org.example.storemanager.entity.sales.PurchaseOrderDetail;
...
import org.example.storemanager.repository.sales.PurchaseOrderDetailRepository;
import org.example.storemanager.repository.wms.PurchaseRequestRepository;
import org.example.storemanager.repository.warranty.SupplierContractRepository;
...
public class PurchaseSupportController {
    ...
    public ResponseEntity<ApiResponse<Supplier>> getSupplierById(@PathVariable Long id) {
        Supplier supplier = supplierRepository.findById(id)
                .filter(s -> !s.getIsDeleted())
                .orElseThrow(() -> new org.example.storemanager.exception.ResourceNotFoundException("Supplier", "id", id));
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getSuppliersDropdown(@RequestParam(required = false) String keyword) {
        List<Supplier> suppliers = supplierRepository.findAll().stream()
                .filter(s -> !s.getIsDeleted() && (keyword == null || s.getName().toLowerCase().contains(keyword.toLowerCase())))
                .collect(Collectors.toList());
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getProductsDropdown(@RequestParam(required = false) String keyword) {
        List<Product> products = productRepository.findAll().stream()
                .filter(p -> !p.getIsDeleted() && (keyword == null || p.getName().toLowerCase().contains(keyword.toLowerCase())))
                .collect(Collectors.toList());
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getVariantsDropdown(@RequestParam(required = false) String keyword) {
        List<ProductVariant> variants = productVariantRepository.findAll().stream()
                .filter(v -> !v.getIsDeleted() && (keyword == null || v.getSku().toLowerCase().contains(keyword.toLowerCase())))
                .collect(Collectors.toList());
        ...
        List<Map<String, Object>> result = variants.stream().map(v -> {
            ...
            map.put("name", v.getProduct().getName());
            return map;
        }).collect(Collectors.toList());
    ...
    public ResponseEntity<ApiResponse<PurchaseDashboardSummaryResponse>> getDashboardSummary() {
        ...
        long pendingRequests = purchaseRequestRepository.findAll().stream()
                .filter(r -> !r.getIsDeleted() && ("PENDING_APPROVAL".equalsIgnoreCase(r.getStatus()) || "SUBMITTED".equalsIgnoreCase(r.getStatus())))
                .count();

        long activeContracts = supplierContractRepository.findAll().stream()
                .filter(c -> !c.getIsDeleted() && "ACTIVE".equalsIgnoreCase(c.getStatus()))
                .count();
        ...
        List<PurchaseOrderDetail> poDetails = purchaseOrderDetailRepository.findAll().stream()
